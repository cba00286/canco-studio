...
- **이미지 생성 작업지**가 화마다 있다 — `episodes/<ep>/prompts/이미지_작업지.md`.
  `scripts/build_image_list.py --episode ep2` 로 다시 만든다. 컷 순서대로 캐릭터 지정·
  프롬프트·시드가 한 파일에 늘어서 있어서, 화면을 오가지 않고 위에서부터 훑으며 작업한다
...
- **나레이션은 화면에 굽지 않는다.** `.srt`(유튜브 CC)에만 넣는다. `--narration-burn` 으로 켠다
- **쇼츠에는 소리 글자(「쿵!」「반짝!」)를 띄운다.** 소리 끄고 보는 사람이 대부분이다.
  `subtitle_style_shorts.json` 이 기본으로 켜 둔다. 본편은 꺼져 있다
- **자막 위치는 `measure_speech.py` 로 재서 맞춘다.** 컷 길이에서 계산하면 반드시 어긋난다 (`docs/12`)
